...
Administrators can `PATCH /items/:id/metadata` to edit and lock fields, `PUT /items/:id/match` with `{ "tmdbId": "123" }` to correct a movie or series match, and `POST /items/:id/refresh` to queue TMDb enrichment. Set `LUMEN_TMDB_API_KEY` to a TMDb v3 API key on the server to enable remote metadata; restarting the server then queues enrichment for previously scanned video items. Scanning and local browsing work without it.
