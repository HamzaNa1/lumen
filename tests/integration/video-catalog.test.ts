...
test("video folders browse as series, seasons, episodes and movies without required sidecars", async () => {
  ...
  for (const path of [
    join(showsRoot, "House", "Season 1"),
    join(showsRoot, "House", "Season 2"),
    join(moviesRoot, movieFolder),
  ])
    await mkdir(path, { recursive: true });
  const episodes = [
    "House/Season 1/House S01E01.mkv",
    "House/Season 1/House S01E02.mkv",
    "House/Season 2/House S02E01.mkv",
  ];
  ...
  const ids = await Effect.runPromise(
    ...
    `);
      for (const [id, name, kind, rootId, path] of [
        [showsId, "Shows", "shows", showRootId, showsRoot],
        [moviesId, "Movies", "movies", movieRootId, moviesRoot],
      ]) {
        yield* db.run(
          sql`INSERT INTO libraries(id, name, slug, is_enabled, created_at_ms, updated_at_ms) VALUES (${id}, ${name}, ${kind}, 1, 1, 1)`,
        );
        yield* db.run(sql`INSERT INTO library_profiles(library_id, kind) VALUES (${id}, ${kind})`);
        yield* db.run(
          sql`INSERT INTO library_roots(id, library_id, path, is_enabled, priority, created_at_ms, updated_at_ms) VALUES (${rootId}, ${id}, ${path}, 1, 0, 1, 1)`,
        );
        yield* db.run(
          sql`INSERT INTO library_root_states(root_id, library_id, canonical_path, canonical_key, is_available, scan_generation, updated_at_ms) VALUES (${rootId}, ${id}, ${path}, ${path}, 1, 1, 1)`,
        );
      }
  ...
  const shows = (await (await get(`/api/v1/items?libraryId=${ids.showsId}`, admin)).json()) as {
    items: { id: string; title: string; kind: string; artworkId: string | null }[];
  };
  ...
  const seasons = (await (await get(`/api/v1/items/${showId}/children`, admin)).json()) as {
    items: { id: string; title: string; kind: string }[];
  };
  expect(seasons.items.map((item) => item.title)).toEqual(["Season 1", "Season 2"]);
  const firstSeason = (await (
    await get(`/api/v1/items/${must(seasons.items[0]).id}/children`, admin)
  ).json()) as { items: { id: string; indexNumber: number }[] };
  expect(firstSeason.items.map((item) => item.indexNumber)).toEqual([1, 2]);
  const movies = (await (await get(`/api/v1/items?libraryId=${ids.moviesId}`, admin)).json()) as {
    items: { id: string; title: string; year: number; kind: string }[];
  };
  expect(movies.items).toEqual([
    expect.objectContaining({ title: "Movie Name", year: 2021, kind: "movie" }),
  ]);
  await writeFile(
  ...
  const artworkRow = await Effect.runPromise(
    Effect.gen(function* () {
  ...
  expect(artworkRow?.contentHash).toBe(
  ...
  const nextUpBefore = (await (await get(`/api/v1/items/${showId}/next-up`, admin)).json()) as {
    item: { id: string };
  };
  expect(nextUpBefore.item.id).toBe(must(firstSeason.items[0]).id);
  const watched = await fetch(
    new URL(`/api/v1/items/${must(firstSeason.items[0]).id}/watch-state`, base),
    {
      method: "PUT",
      headers: { authorization: `Bearer ${admin}`, "content-type": "application/json" },
      body: JSON.stringify({ positionSeconds: 60, completed: true }),
    },
  );
  expect(watched.status).toBe(200);
  const nextUpAfter = (await (await get(`/api/v1/items/${showId}/next-up`, admin)).json()) as {
    item: { id: string };
  };
  ...
  const viewerNextUp = (await (await get(`/api/v1/items/${showId}/next-up`, viewer)).json()) as {
    item: { id: string };
  };
  ...
  const favorite = await fetch(
    new URL(`/api/v1/items/${must(firstSeason.items[0]).id}/favorite`, base),
    {
      method: "PUT",
      headers: { authorization: `Bearer ${admin}`, "content-type": "application/json" },
  ...
  const rescanned = (await (await get(`/api/v1/items?libraryId=${ids.showsId}`, admin)).json()) as {
  ...
  const progress = (await (
    ...
    watchState: { completed: boolean };
    isFavorite: boolean;
    sources: ReadonlyArray<{ available: boolean }>;
  };
  expect(progress.watchState.completed).toBe(true);
  expect(progress.isFavorite).toBe(true);
  expect(progress.sources[0]?.available).toBe(true);
  await server.stop();
  servers.pop();
  await rename(
    join(showsRoot, must(episodes[0])),
    join(showsRoot, "House/Season 1/House S01E01 Renamed.mkv"),
  );
  await Effect.runPromise(
    Effect.gen(function* () {
      const db = yield* Database;
      ...
      const runId = newUuid();
      yield* db.run(
        sql`INSERT INTO scan_runs(id, library_id, mode, status, started_at_ms, created_at_ms) VALUES (${runId}, ${ids.showsId}, 'full', 'running', 1, 1)`,
      );
      ...
      yield* db.run(
        sql`UPDATE scan_runs SET status = 'succeeded', finished_at_ms = 1 WHERE id = ${runId}`,
      );
    }).pipe(Effect.provide(layer)),
  );
  ...
  try {
    const enrichedServer = await startServer({
      databasePath,
      dataDir: join(root, "data"),
      host: "127.0.0.1",
      port: 0,
    ...
    const remoteGet = (path: string) =>
      fetch(new URL(path, remoteBase), { headers: { authorization: `Bearer ${admin}` } });
    ...
    expect(
      (await fetch(settingPath, { headers: { authorization: `Bearer ${viewer}` } })).status,
    ).toBe(403);
    expect(
      (
        await fetch(settingPath, {
          method: "PUT",
          headers: { authorization: `Bearer ${viewer}`, "content-type": "application/json" },
          body: JSON.stringify({ tmdbApiKey: "test" }),
        })
      ).status,
    ).toBe(403);
    const savedSetting = await fetch(settingPath, {
      method: "PUT",
      headers: { authorization: `Bearer ${admin}`, "content-type": "application/json" },
      body: JSON.stringify({ tmdbApiKey: "test" }),
    });
    ...
    expect(
      (
        (await (await remoteGet(`/api/v1/items/${showId}`)).json()) as {
          metadataProviderConfigured: boolean;
        }
      ).metadataProviderConfigured,
    ).toBe(true);
    const movedEpisodes = (await (
      await remoteGet(`/api/v1/items/${must(seasons.items[0]).id}/children`)
    ).json()) as { items: { id: string }[] };
    expect(movedEpisodes.items.map((episode) => episode.id)).toEqual(
      firstSeason.items.map((episode) => episode.id),
    );
    ...
    const enrichedShow = (await (await remoteGet(`/api/v1/items/${showId}`)).json()) as {
      item: { title: string; overview: string; year: number; communityRating: number };
    };
    ...
    expect(
      (
        await fetch(new URL(`/api/v1/items/${must(movies.items[0]).id}/refresh`, remoteBase), {
          method: "POST",
          headers: { authorization: `Bearer ${admin}` },
        })
      ).status,
    ...
    for (let attempt = 0; attempt < 40; attempt++) {
      movieOverview = (
        (await (await remoteGet(`/api/v1/items/${must(movies.items[0]).id}`)).json()) as {
          item: { overview: string | null };
        }
      ).item.overview;
    ...
    const enrichedSeasons = (await (
      await remoteGet(`/api/v1/items/${showId}/children`)
    ).json()) as { items: { artworkId: string | null }[] };
    expect(must(enrichedSeasons.items[0]).artworkId).toBeString();
    expect(
      (
        await fetch(
          new URL(`/api/v1/artwork/${must(enrichedSeasons.items[0]).artworkId}`, remoteBase),
    ...
    for (let attempt = 0; attempt < 40; attempt++) {
      correctedYear = (
        (await (await remoteGet(`/api/v1/items/${showId}`)).json()) as { item: { year: number } }
      ).item.year;
    ...
    await writeFile(
      join(showsRoot, "House", "tvshow.nfo"),
      '<tvshow><title>House</title><plot>A local synopsis.</plot><uniqueid type="tmdb">123</uniqueid></tvshow>',
    );
    await Effect.runPromise(
      Effect.gen(function* () {
        yield* (yield* makeMediaIngest).ingest(must(ids.sourceIds[1]));
      }).pipe(Effect.provide(layer)),
    );
    const correctedDetails = (await (await remoteGet(`/api/v1/items/${showId}`)).json()) as {
      item: { externalIdsJson: string };
    };
    ...
    const withClinic = (await (
      await remoteGet(`/api/v1/items?libraryId=${ids.showsId}`)
    ).json()) as { items: { title: string; artworkId: string }[] };
    const clinicArtworkId = must(
      withClinic.items.find((entry) => entry.title === "Clinic"),
    ).artworkId;
    ...
    expect(
      (
        await fetch(new URL(`/api/v1/items/${showId}/refresh`, remoteBase), {
          method: "POST",
          headers: { authorization: `Bearer ${admin}` },
        })
      ).status,
    ...
    const restartedSettings = new URL(
      "/api/v1/admin/metadata-settings",
      restartedServer.server.url,
    );
    expect(
      await (
        await fetch(restartedSettings, { headers: { authorization: `Bearer ${admin}` } })
      ).json(),
    ).toEqual({ tmdbConfigured: true });
    const cleared = await fetch(restartedSettings, {
      method: "PUT",
      headers: { authorization: `Bearer ${admin}`, "content-type": "application/json" },
    ...
    expect(
      (
        (await (
          await fetch(new URL(`/api/v1/items/${showId}`, restartedServer.server.url), {
            headers: { authorization: `Bearer ${admin}` },
          })
        ).json()) as { metadataProviderConfigured: boolean }
      ).metadataProviderConfigured,
    ).toBe(false);
  } finally {
