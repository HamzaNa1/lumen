...
import { ScheduledJobService, ScheduledJobServiceLive } from "../../apps/server/src/jobs/ScheduledJobService";
...
describe("durable jobs and scanner reconciliation", () => {
  ...
  test("schedules the library watcher and scans a library when a root modification time changes", async () => {
    ...
    const databaseLayer = makeDatabaseLayers({ databasePath: join(workspace, "server.sqlite") } as never);
    const repositories = RepositoriesLive(databaseLayer);
    ...
    const scheduler = ScheduledJobServiceLive.pipe(Layer.provide(Layer.mergeAll(dependencies, watcher)));
    ...
    const result = await Effect.runPromise(Effect.gen(function* () {
      ...
      yield* database.run(sql`INSERT INTO library_roots(id, library_id, path, is_enabled, priority, created_at_ms, updated_at_ms) VALUES (${rootId}, ${libraryId}, ${root}, 1, 0, 1, 1)`);
      yield* database.run(sql`INSERT INTO library_root_states(root_id, library_id, canonical_path, canonical_key, is_available, scan_generation, updated_at_ms) VALUES (${rootId}, ${libraryId}, ${root}, ${root}, 1, 0, 1)`);
      ...
      const runs = yield* database.all<{ mode: string; status: string }>(sql`SELECT mode, status FROM scan_runs WHERE library_id = ${libraryId}`);
      const jobs = yield* database.all<{ operation: string; dedupeKey: string }>(sql`SELECT operation, dedupe_key AS dedupeKey FROM scan_jobs`);
      const schedule = yield* database.get<{ nextRunAtMs: number; lastFinishedAtMs: number; lastError: string | null }>(sql`
      ...
      `);
      return { firstSchedule, earlySchedule, scansStarted, repeated, afterIntervalChange, runs, jobs, schedule };
    }).pipe(Effect.provide(layer)));
  ...
  test("defers a changed library while another scan is active", async () => {
    ...
    const result = await Effect.runPromise(Effect.gen(function* () {
      ...
      const active = yield* repos.scanning.startRun({ runId: newUuid(), libraryId, mode: "full", startedAtMs: 150 });
      yield* Effect.promise(() => mkdir(join(root, "new-show")));
      const deferred = yield* libraryWatcher.check(200);
      yield* repos.scanning.finishRun({ runId: active.id, status: "succeeded", nowMs: 250, errorCode: null, errorMessage: null });
      const started = yield* libraryWatcher.check(300);
      const runCount = yield* database.get<{ count: number }>(sql`SELECT count(*) AS count FROM scan_runs WHERE library_id = ${libraryId}`);
